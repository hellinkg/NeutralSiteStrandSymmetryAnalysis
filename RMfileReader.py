...
name = re.compile("^C?\s*(\S*(?:[^\s\d]|(?:\d\s)))\s*\d")  # This can add an extra space to the name

float_re = "(\d+\.\d+)"
top_line_re = re.compile("(\d+)\s+" + float_re + "\s+" + float_re + "\s+" +
                         float_re + "\s+(chr\d+|\d+)\s+(\d+)\s+(\d+)\s+\((\d+)\)\s+(C\s+|\+\s+|\s*)" #(C?|\+?)\s+"
                         + "(\([A-Z]+\)n|[A-Z]+_rich|[A-z0-9\-]+|[A-Z|a-z|0-9]+_|[A-Z|a-z|0-9|-]+_[A-Z|a-z|0-9]+|"
                         + "[A-z]+-[0-9]+_[A-z]+-[A-z]+|[A-z]+-[0-9]+_[A-z]+-|[A-z|0-9]+_[A-z]+_[A-z]+|[A-z|0-9]+_[A-z]+_[A-z]+|\w+\?|[A-z|0-9]+\?/[A-z|0-9]+\?)"
                         + "(#|\s+)(\w+|\w+/\w+_type|Low_complexity|Simple_repeat|\w+/\w+|[A-z]+/[A-z\-0-9]+|"
                         + "[A-z]+-[0-9]+_[A-z]+-[A-z]+|[A-z]+-[0-9]+_[A-z]+-|[A-z|0-9]+_[A-z]+_[A-z]+|[A-z|0-9]+_[A-z]+_[A-z]+|\w+\?|[A-z|0-9]+\?/[A-z|0-9]+\?)\s+" 
                         + "\(?(\d+)\)?" + "\s+(\d+)\s+\(?(\d+)\)?\s+(\d+)\s*")
complement_re = re.compile("^C?\s+[A-z\-_\#0-9/\?\(\)]+\s+\d+")
first_lineAL = re.compile("^\s+((chr)|\d+\s+\d+\s+[A-z\-]+)")
comp_line = re.compile("^C?\s+[A-z\-_\#0-9/\?\(\)]+\s+\d+")
rm_prnth = re.compile("^\(")
out_header = re.compile("^[A-Z|a-z]|^\s+[A-Z|a-z]")
...
class AlignFile:
    ...
    retrieves all of the information from each data entry. Each entry usually 
    ...
    def __init__(self, fp):
        ...
        complement strand), the repeat's class and family, the repeat's 
    ...
    def readAlignFile(self, fp):
        ...
        .align file. It groups the first line into a regular expression (if 
        ...
        and then breaks to finish the Align object for this data entry.
        """

        chrAL = "" # Assists in attaining the modern (queried) sequence
        CchrAL = "" # Assists in attaining the ancestral sequence
        firstLineFlag = False # Indicates if first standard line exists
        atypStartCountQ = False # Indicates if the atypical start of an entry has been found yet (modern/query sequence) 
        atypStartCountC = False # Indicates if the atypical start of an entry has been found yet (ancestral sequence)
        ...
        for line in fp:
            ...
            if re.search(top_line_re, line.lstrip()):
                startLine = re.search(top_line_re, line.lstrip())
                ...
                self.posQueryBeg = int(startLine.group(6))-1
                self.posQueryEnd = int(startLine.group(7))
                self.leftQuery = int(startLine.group(8))
                
                self.matchRepeat = startLine.group(10)
                if re.search(rm_endsName, self.matchRepeat):
                    self.matchRepeat = self.matchRepeat[:-5]
                self.repClassFam = startLine.group(12)
                     
                # If "+"
                if startLine.group(9) == "" or startLine.group(9).rstrip() == "+":
                    self.posOrC = "+"
                ...
                # Lineage identification number
                self.info = startLine.group(16)
                
                firstLineFlag = True
        
            # Looks for the query sequence if the standard first line is missing
            elif re.match(first_lineAL,line) and firstLineFlag == False:
            ...
            # Looks for the complement string if the typical first line is missing
                ...
                if group2[0] == "C":
                    ...
                    if atypStartCountC == False and name.search(line):
                        ...
                        if not getNames.find("#") == -1 and not getNames.find("#") == len(seqInfo.group(1).rstrip())-1:
                            ...
                            self.matchRepeat = getNames.split("#",1)[0]
                            self.repClassFam = getNames.split("#",1)[1]
                            ...
                            self.matchRepeat = getNames.split("-",1)[0]
                            self.repClassFam = getNames.rstrip("#")                                             
            ...
            elif re.match(first_lineAL,line) and firstLineFlag == True:
                if r_coords.search(line):
                    r = r_coords.search(line)   
                    chrAL = chrAL + r.group(2)
                else:
            ...
            elif re.search(comp_line,line) and firstLineFlag == True:
                if r_coords.search(line):
                    r = r_coords.search(line)
                    CchrAL = CchrAL + r.group(2)                        
                else:
...
class OutFile:
    ...
    def readOutFile(self, outStr):
        ...
        """
        This method is used for parsing the .out string. It splits
        ...
        Note: This method will call the castToInt method to remove the parenthesis 
        and cast the number to an int for readability purposes.
        """
    ...
    def castToInt(self,data):
        ...
        parathesis surrounding it and casts it to an int.
        """
...
class Repeat:
    ...
    def __init__(self, alignObj = None, outObj = None):
        ...
        """
        This __init__ takes in two arguments - the object created 
        from the .align file and the object from the .out file. If 
        the Align or Out objects are not given, then it sets all attributes 
        to None. Otherwise, it calls the method setRepObj to begin setting 
        the data found from both files in order to create the complete Repeat
        data object.
        """    
    ...
    def ancestor_coords(self, begin = None, end = None):
        """Set/Return alignment cords w.r.t. sequences."""
        if begin != None:
    ...
    def modern_chr(self, seq_name = None):
        """Set/Return name of chromosome / sequence containing modern instance"""
        if seq_name != None:
...
def OutFileGenerator(outFile, excludeSet = {}):
    """
    This method creates an OutFile object and skips
    over any header or any blank lines. Also will account
    for any exclusions given.
    outFile: Current .out file being parsed.
    excludeSet: (Optional) TE family instances to be ignored.
    """
    fp = open(outFile)
    outObjs = []

    for line in fp:
        if not re.match(out_header,line) and line.rstrip():
            O = OutFile(line)
            if not excludeSet or (O.repClassFam not in excludeSet and O.matchRepeat not in excludeSet):
                yield O
                
def openBZ2(file):
    """
    Opens a .bz2 file based on the Python version
    of the user.
    file: Current .align file.
    """
    if sys.version_info[0] == 2:
...
def AlignFileGenerator(alignFile, excludeSet = {}):
    """
    Opens the .align file and parses all data into 
    AlignFile objects.
    alignFile: Current .align file being parsed.
    excludeSet: (Optional) TE family instances to be ignored.
    """
    ...
    while 1:
        ...
        if str(A) == "0#0##":
            break
        if not excludeSet or (A.repClassFam not in excludeSet and A.matchRepeat not in excludeSet):
            yield A

def createRepObjs(alignFile, outFile, excludeFile = None):
    """
    Creates a list of Repeat objects based on the .out and .align
    file data. Also incorporates any exclusions that may need to be
    taken into account. Will use the longest repeat in order to create
    the Repeat object for duplicate data.
    alignFile: Current .out file being parsed.
    outFile: Current .out file being parsed.
    excludeFile: (Optional) File that contains TE family instances to be ignored.
    """
    excludeSet = {line.rstrip() for line in excludeFile} if excludeFile else {}
    alignObjs = list(AlignFileGenerator(alignFile, excludeSet))
    outObjs = OutFileGenerator(outFile, excludeSet)
...
def pickleRepeats(repeatList, fileName):
    ...
    repeatList: List of Repeat objects created.
...
def create_prmFile(alignFile, outFile, prmFile, excludeFile):
    """
    This method creates a .prm file from the .align and .out
    files given. It will also pass on any exclusions to the
    createRepObjs method for use.
    alignFile: Current .out file being parsed.
    outFile: Current .out file being parsed.
    prmFile: Output file name to contain pickled objects.
    excludeFile: (Optional) File that contains TE family instances to be ignored.
    """
...
def prm_generator(chr, dir = "."):
    """
    This method generates unpickled Repeat objects 
    from the designated version number of Python attached.
    By default, it will look in the current directory for this
    file name. 
    chr: Chromosome file prefix.
    dir: Directory where .prm files are located.
    """
...
def unpickleRepeats(chr, dir = "."):
    ...
    Calls the prm_generator method to create
    a list of unpickled Repeat objects.
    chr: Chromosome file prefix.
    dir: Directory where .prm files are located.
    """
    return list(prm_generator(chr, dir))
                
def beginReadingFiles(args):
    """
    Organizes the output from the command line to execute
    the library code based on user's requirements.
    args: Console arguments.
    """
    if args.pickleDecision == 1:
        bz2Option = ""
        if args.bz2Option == 1:
            bz2Option = ".bz2"
        if args.chrDirectory == '' and (args.chrRangeBegin == 1 and args.chrRangeEnd == 22):
            for file in glob.glob(args.chrDirectory + args.rmPrefix + "*.fa.out"):
                if re.match(args.rmPrefix + "\d+\.fa*", file):
                    file = file.rstrip(".fa.out")
                    print("Converting: %s" % (file))
                    create_prmFile(file + ".fa.align" + bz2Option, file + ".fa.out", args.prmDirectory + file + ".%d.prm" % (sys.version_info[0]), args.excludeFile)
        else:
            for chrNum in range(args.chrRangeBegin, args.chrRangeEnd+1):
                file = args.rmPrefix + "%d" % chrNum#"Zea_mays.AGPv3.27.dna.chromosome.%d" % chrNum#"Arabidopsis_thaliana.TAIR10.27.dna.chromosome.%d" % chrNum#"Zea_mays.AGPv3.27.dna.chromosome.%d" % chrNum#"chr%d" % chrNum
                print("Converting: %s" % (file))
                create_prmFile(args.chrDirectory + file + ".fa.align" + bz2Option, args.chrDirectory + file + ".fa.out", args.prmDirectory + file + ".%d.prm" % (sys.version_info[0]), args.excludeFile)
    else:
        [unpickleRepeats(args.rmPrefix + "%d" % chrNum, args.prmDirectory) for chrNum in range(args.chrRangeBegin, args.chrRangeEnd+1)]


def commandLineReader():
    """
    Sets up a command line parser that allows the user to 
    have less interaction with the code and more options to
    create the .prm files. Describes usage with the '-h'/'--help'
    command.
    """
    usage = "usage: RMfileReader.py [--ex] fileName.txt [--beg] B [--end] E [--bz2] 0/1 [--rmDir] " + \
            "/Where/The/RepeatMasker/Files/Are/Located/ [--prmDir] /Where/The/Prm/Files/Are/Located/ [--rmPrefix] OrganismFilePrefix [--pickle] 0/1"
    commandParser = argparse.ArgumentParser(usage=usage)
    commandParser.add_argument('--ex', type=argparse.FileType('r'), default=open("hg18.rpt_exclude.txt",'r'), 
                               dest='excludeFile',
                               help="File of listed repeat class family to exclude from .prm creation (.txt, path required if not in current directory, not a required argument)" )
    commandParser.add_argument('--beg', type=int, default=1, 
                               dest='chrRangeBegin',help="Beginning of range for desired .prm files (inclusive, not required)")
    commandParser.add_argument('--end', type=int, default=22, 
                               dest='chrRangeEnd',help="Ending of range for desired .prm files (inclusive, not required)")
    commandParser.add_argument('--bz2', type=int, default=1,
                               choices=range(0,2), dest='bz2Option',
                               help="Are the align files in .bz2 format? (Default is yes.)")
    commandParser.add_argument('--rmDir', default='',dest='chrDirectory',
                               help="Directory of where the .fa.out and .fa.align/.fa.align.bz2 files exist (not required if in current directory)")
    commandParser.add_argument('--prmDir', default='',dest='prmDirectory',
                               help="Directory of where the prm files will exist (not required if wanted inside current directory)")
    commandParser.add_argument('--rmPrefix', default='chr',dest='rmPrefix',
                               help="Prefix for the .align and .out files. Anything up to the chromosome number in those files.")
    commandParser.add_argument('--pickle', type=int, default=1,
                               choices=range(0,2), dest='pickleDecision',
                               help="Do you want to pickle or unpickle these files? (Default is set to pickle.)")
    beginReadingFiles(commandParser.parse_args())
